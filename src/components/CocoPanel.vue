...
let currentCall: any = api.getLatestCocoImages
...
function getImageAsStyleBackgroundAttr(cocoImage: CocoImage) {
  if (cocoImage.gif !== null || cocoImage.coloredGif !== null) {
    ...
    const svg64 = window.btoa(cocoImage.coloredSvgContent || cocoImage.svgContent || '')
    return {
...
<template>
  <div class="host">
    <form class="search-form" v-on:submit.prevent="handleClickOnSearchButton()">
      ...
      <button type="submit" data-appearance="primary">Search</button>
    </form>

    <div class="result-container">
      <div class="result-list" v-if="results">
        <button
          class="item"
        ...
        >
          <div
            class="coco-image"
            :alt="item.name"
            :style="getImageAsStyleBackgroundAttr(item)"
          ></div>
        </button>
      </div>

      <div class="show-more" v-if="hasMorePages">
        <button
          type="button"
      ...
      <div class="no-results" v-if="results.length === 0 && text">
        <p class="body-m">There is nothing for your search.<br />Try something else.</p>
      </div>
    </div>
  </div>
</template>
